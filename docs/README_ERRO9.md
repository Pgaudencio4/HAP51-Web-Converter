...
O erro "Subscript out of range" (Error 9) no HAP 5.1 ocorre durante a simulação quando o programa tenta aceder a um índice de array inválido.
...
### 1. Usar o validador/corrector automático:
...
### 2. Correcção manual (se necessário):
...
# Também corrigir Default Space
spc_data = bytearray(files['HAP51SPC.DAT'])
for offset in [554, 560, 566, 594, 616, 660]:
    spc_data[offset:offset+2] = struct.pack('<H', 0)
files['HAP51SPC.DAT'] = bytes(spc_data)
...
1. **Não usar schedules com calendário corrompido** - usar sempre o modelo base (HAP_Template_Clean.E3A)

2. **Validar após conversão** - executar sempre `validar_e3a.py` após gerar um ficheiro

3. **Offsets 554, 560, 566 são Schedule IDs** - não são flags! O código original estava errado:

```python
# ERRADO:
struct.pack_into('<H', data, 554, 2)  # "Flag ACH mode" <- NÃO!

# CORRECTO:
struct.pack_into('<H', data, 554, 0)  # Schedule ID (0 = Sample Schedule)
```
...
|----------|-----------|--------|
| HAP_Template_Clean.E3A | Template original limpo | ✓ Funciona |
| Base_Vazio.E3A | Template vazio | ✓ Funciona |
| CasaAlecrim2025_Prev (1).E3A | Exemplo funcional | ✓ Funciona |
| Com erro em simul.E3A | Ficheiro original com erro | ✗ Erro 9 |
| Com erro em simulv5.E3A | Ficheiro corrigido | ✓ Funciona |
...
| Script | Descrição |
|--------|-----------|
| `validar_e3a.py` | Validador e corrector automático |
| `excel_to_hap.py` | Conversor Excel -> HAP (actualizado) |
...
**Data:** 2026-02-04
**Autor:** Análise técnica do formato HAP 5.1
