...
def extract_walls_assemblies(wal_data):
    """Extrai nomes dos wall assemblies"""
...
def extract_roofs_assemblies(rof_data):
    """Extrai nomes dos roof assemblies"""
...
def main():
    ...
    walls = extract_walls_assemblies(files.get('HAP51WAL.DAT', b''))
    roofs = extract_roofs_assemblies(files.get('HAP51ROF.DAT', b''))
    windows = extract_windows(files.get('HAP51WIN.DAT', b''))
    ...
    # Criar Excel
    create_excel(spaces, schedules, walls, roofs, windows, output_file)

    print(f"\nFicheiro criado: {output_file}")
    print("Formato: 147 colunas (mesmo que HAP_Template_RSECE.xlsx)")
